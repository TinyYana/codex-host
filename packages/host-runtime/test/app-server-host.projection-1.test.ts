...
describe("AppServerHost HarnessAdapter projection", () => {
  ...
  it.each(["codexhost/account/switch", "codexhost/account/delete", "codexhost/account/recover"])(
    "refuses Host account management %s on a read-only Account control",
    async (methodName) => {
      ...
      try {
        ...
        await expect(fixture.collector.waitFor((message) => message.id === 911)).resolves.toEqual({
          id: 911,
          error: {
            code: -32086,
            message: "Unknown Codex Account",
  ...
  // Host never owns login, logout, or reset-credit consumption.
  it.each([
  ...
  ])("forwards unowned Host account method %s as an unknown method", async (methodName) => {
    const fixture = createFixture();
    try {
      await fixture.ready;
