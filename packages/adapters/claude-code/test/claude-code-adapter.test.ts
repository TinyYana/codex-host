...
class FakeClaudeTransport implements ClaudeTurnTransport {
  ...
  runTurn(
    ...
    if (text.startsWith("/goal ")) {
      if (text === "/goal clear") this.clearGoalCalls.push(userMessageId);
      else this.goalCalls.push({ objective: text.slice("/goal ".length), userMessageId });
    }
...
describe("Claude Code native Goal", () => {
  ...
  it("releases a transport when the Goal acknowledgement times out", async () => {
    vi.useFakeTimers();
    try {
      const { adapter, transports } = fixture();
      const session = await openSession(adapter);
      ...
      const setting = goal.set({
        turnId: hostTurnIdSchema.parse("timed-out-goal"),
        objective,
      });
      ...
      await expect(session.execute(textTurn("after-timeout"))).resolves.toMatchObject({ ok: true });
      expect(transports).toHaveLength(2);
      transports[1]?.finish({ status: "succeeded" });
      await session.close();
    } finally {
  ...
  it("starts after native acknowledgement and waits for the terminal transcript record", async () => {
  ...
  it("clears the native Goal without projecting a Turn", async () => {
    ...
    transport.finish({ status: "succeeded" });
    await expect(clearing).resolves.toEqual({ ok: true, value: true });
    expect(await nextEvent(iterator)).toEqual({
      type: "session.goal.changed",
      goal: null,
      outcome: "cleared",
    });
    await expect(goal.read()).resolves.toEqual({ ok: true, value: null });
    await session.close();
  });

  it("settles a pending Goal command immediately on an unrecoverable-error notice", async () => {
    const { adapter, transports } = fixture();
    const session = await openSession(adapter);
    const iterator = session.outputs[Symbol.asyncIterator]();
    const goal = session.goal;
    ...
    const setting = goal.set({ turnId: hostTurnIdSchema.parse("errored-goal"), objective });
    expect((await nextEvent(iterator)).type).toBe("session.state.changed");
    const transport = transports[0];
    if (!transport) throw new Error("Fake Claude transport was not created");
    await vi.waitFor(() => expect(transport.goalCalls).toHaveLength(1));
    transport.goalSignal({
      type: "clearedByError",
      reason: 'Goal cleared after an unrecoverable error (model unavailable): "x".',
    });
    transport.finish({ status: "succeeded" });
    // No 15s acknowledgement timeout: the error settles the command directly.
    await expect(setting).resolves.toMatchObject({ ok: false, error: { code: "nativeFailure" } });

    const turnId = hostTurnIdSchema.parse("after-error");
    await expect(session.execute(textTurn("after-error"))).resolves.toEqual({
      ok: true,
      value: { turnId },
    });
    expect(await nextEvent(iterator)).toEqual({ type: "turn.started", turnId });
  ...
  it("passes session-scoped events through while a Goal acknowledgement is pending", async () => {
    const { adapter, transports } = fixture();
    const session = await openSession(adapter);
    const iterator = session.outputs[Symbol.asyncIterator]();
    const goal = session.goal;
    ...
    const setting = goal.set({ turnId: hostTurnIdSchema.parse("busy-goal"), objective });
    expect((await nextEvent(iterator)).type).toBe("session.state.changed");
    const transport = transports[0];
    if (!transport) throw new Error("Fake Claude transport was not created");
    await vi.waitFor(() => expect(transport.goalCalls).toHaveLength(1));
    // A Session-scoped change must not wait behind the pending acknowledgement.
    transport.changePermissionMode("acceptEdits");
    expect(await nextEvent(iterator)).toMatchObject({ type: "session.state.changed" });
    transport.goalSignal({
      type: "command",
      output: "/goal can't run while hooks are restricted.",
    });
    transport.finish({ status: "succeeded" });
    await expect(setting).resolves.toMatchObject({ ok: false, error: { code: "unsupported" } });
    await session.close();
