...
3. **Switch transaction.** `changing` phase returns to `OfficialWorkGate`; admission during a switch fails fast as busy. Steps: assert file-based storage and no API-key env override → acquire change lease (busy if active work; bounded drain — close new admissions, wait ≤10s for in-flight official requests, abort the switch on timeout without stopping the backend) → capture current `auth.json` (refresh stored copy) → stop owned backend, awaiting exit proof → re-capture (token rotation during stop) → atomic install of target credential → start backend → verify via `account/read` polling (~200ms, ≤10s) plus permanent-file identity comparison → publish new current. On verify/start failure: reinstall the captured credential and re-verify; if the on-disk credential now holds a newer generation of the same account, keep it (cc-switch's newer-generation rule). If rollback itself fails, phase becomes `unavailable` with `recovery-required`. Stale protection reuses generation retire: responses from the old backend generation return to their original request client and never mutate post-switch state; UI updates are gated on the verified identity read, not on install.
...
- First manage action (save/switch/delete) reads an existing `.codexhost-native-accounts` vault (v1–v3) and rewrites it as v3. Until then the file stays untouched, preserving the removed-feature contract for users who never opt in.
