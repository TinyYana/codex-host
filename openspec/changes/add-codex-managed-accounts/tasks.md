...
- [x] 2.5 Switch transaction (`account/native-codex-accounts.ts`): idle assert, bounded drain, capture/install/verify/rollback, newer-generation preservation, unsafe-external-process detection, fixed-form errors, current derivation.
...
- [x] 5.2 Switch transaction: success, busy refusal, drain timeout, verify-failure rollback, rollback preserving newer generation, rollback failure → unavailable, stale/late response isolation, external-process unsafe error, concurrent switch requests.
